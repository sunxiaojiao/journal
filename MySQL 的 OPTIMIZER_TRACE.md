...
```
查询优化器分析得出两条执行路径，一个是全表扫描。一个是走idx_a_b_d索引，进一步计算得到全表扫描的代价为3.3，走idx_a_b_d索引的代价为6.61。这个结果很正常，可以看到如果选择走索引，需要在B+Tree找到符合条件的行，然后去表中取出，才能把结果行跑出来，因为现在数据就一条，自然走全表的代价更小，随着数据的增加，走索引的代价会比全表的代价越来越小，到时候自然会选择走索引。